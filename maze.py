...
import numpy as np
import humint as hi
from mazesprites import Cell, Wall, Floor, Ball, GetKvColor, Goal
...
# Cool colors:
#   175/256,176/256,148/256,1,'rgba'
#   .084, .098, .120, .9, 'rgba'
...
class MazeGame(Widget):
    ...
    def start(self):
        ...
        mg = EllerMazeGenerator(self.gridWidth, self.gridHeight)
        ...
        ##print('x',playfield.ball.pos[0],' y',playfield.ball.pos[1],sep='')
...
class Playfield(FloatLayout):
    ...
    def __init__(self,*args,backColor=(32/256, 32/256, 43/256, .55, 'rgba'),cellColor=(0,0,0,0,'rgba'),wallColor=(0,0,1,1,'rgba'),ballColor=(.7,0,0,1,'rgba'),goalColor=(0,.8,0,1,'rgba'),xoffset = 32,yoffset = 24):
        super().__init__()
        ...
        with self.canvas:
            self.color = GetKvColor(backColor)
    ...
    def DrawMaze(self,maze):
        ...
        for y in range (h):
            for x in range (w):
                ...
                c = Cell(pos=((x*cw)+self.xoffset,(invy*ch)-self.yoffset), size=(cw,ch),colordata=self.cellColor)
                #print('done')
                self.add_widget(c)
                #look up the grid position and see if we need to add a wall and/or floor to the cell
                if (mtrx[x,y,0]):
                    c.add_widget(Wall(pos=c.pos,size=(int(cw/5),int(ch + ch/5)),colordata=self.wallColor)) 
                if (mtrx[x,y,1]):
                    c.add_widget(Floor(pos=c.pos,size=(cw,int(ch/5)),colordata=self.wallColor))
    ...
    def PlaceBall(self,cell):
        ...
        ##print('x=',x,' y=',y,' size=',cell.size,sep='')
        self.ball = Ball(size=(width,height),pos=(x,y),colordata=(self.ballColor),size_hint=(None,None))
    ...
    def PlaceGoal(self,cell):
        ...
        height = int(cell.size[1] * .5)
        ##print('x=',x,' y=',y,' size=',cell.size,sep='')
        self.goal = Goal(pos=(x,y),size=(width,height),colordata=(self.goalColor))
